...
void loop() {
  ...
  // read sensors into bitmask
  uint8_t m = 0;
  for (int i = 0; i < 3; ++i) m |= (digitalRead(SENS[i]) ? 1 : 0) << i;

  if (m != lastMask) {               // change-only transmit (debounce naturally)
    lastMask = m;
    rs485_send(frame, 3);
  }
